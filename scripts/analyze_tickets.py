#!/usr/bin/env python3
"""Analyze exported tickets and build reports, plots, and a dashboard.
...
- open_priority_summary.csv
- open_eta_buckets.csv
- open_age_buckets.csv
- top_commented_tickets.csv
...
def analyze_tickets(
    ...
    *,
    ...
    open_priority_keys = ["P0", "P1", "P2", "P3"] + sorted(
        key for key in open_priority_counter if key not in {"P0", "P1", "P2", "P3"}
    )
    open_priority_summary_rows = [
        {"priority": key, "open_count": open_priority_counter.get(key, 0)}
        for key in open_priority_keys
    ...
    top_commented_rows = sorted(
        ...
        key=lambda row: (row["comment_count"], row["updated_at"], row["ticket_id"]),
        reverse=True,
    )
    ...
    if lab_summary_rows:
        top_open_lab = max(lab_summary_rows, key=lambda row: (safe_int(row["open_tickets"]), safe_int(row["tickets_total"])))
        if safe_int(top_open_lab["open_tickets"]) > 0:
            insights.append(
                {
                    "title": "Biggest open backlog",
                    "detail": f"{top_open_lab['lab']} has {top_open_lab['open_tickets']} open tickets out of {top_open_lab['tickets_total']} total.",
                }
            )

    if overdue_open_tickets:
        worst_overdue = max(overdue_open_tickets, key=lambda row: safe_int(row["overdue_days"]))
        insights.append(
            {
                "title": "Most overdue ticket",
                "detail": (
                    f"#{worst_overdue['ticket_id'][:8]} in {worst_overdue['lab']} is overdue by "
                    f"{worst_overdue['overdue_days']} days ({worst_overdue['title']})."
                ),
            }
        )

    if open_rows:
        oldest_open = max(open_rows, key=lambda row: safe_int(row["ticket_age_days"]))
        oldest_days = safe_int(oldest_open["ticket_age_days"])
        insights.append(
            {
                "title": "Oldest open ticket",
                "detail": f"#{oldest_open['ticket_id'][:8]} has been open for {oldest_days} days ({oldest_open['title']}).",
            }
        )

    if top_commented_rows:
        hottest = next((row for row in top_commented_rows if row["status"] in OPEN_STATUSES), top_commented_rows[0])
        insights.append(
            {
                "title": "Comment hotspot",
                "detail": (
                    f"#{hottest['ticket_id'][:8]} collected {hottest['comment_count']} comments "
                    f"and is currently in status {hottest['status']}."
                ),
            }
        )
    ...
    if summary["tickets_with_requester_snapshot"] == 0:
        insights.append(
            {
                "title": "No requester snapshots yet",
                "detail": "These exports are mostly older tickets, so requester-vs-admin comparisons will improve on newly created tickets.",
            }
        )
    ...
    return {
        ...
        "lab_summary_rows": sorted(lab_summary_rows, key=lambda row: (safe_int(row["open_tickets"]), safe_int(row["tickets_total"])), reverse=True),
        "category_summary_rows": sorted(
            category_summary_rows, key=lambda row: (safe_int(row["open_tickets"]), safe_int(row["tickets_total"])), reverse=True
        ),
...
def render_html_table(rows: list[dict[str, Any]], columns: list[str]) -> str:
    ...
    head = "".join(f"<th>{html_escape(col.replace('_', ' ').title())}</th>" for col in columns)
    body_rows = []
    for row in rows:
        cells = "".join(f"<td>{html_escape(compact_number(row.get(col)))}</td>" for col in columns)
...
def render_horizontal_bar_chart_svg(
    ...
    *,
    ...
    max_label_chars = max(len(label) for label in labels) if labels else 8

    left_pad = min(290, max(150, max_label_chars * 8))
    right_pad = 84
    top_pad = 86 if subtitle else 64
    bar_height = 26
    gap = 18
    bottom_pad = 28
    ...
    lines = [
        ...
        f'<text x="24" y="34" fill="#0f172a" font-size="22" font-family="Arial, sans-serif" font-weight="700">{html_escape(title)}</text>',
    ]
    if subtitle:
        lines.append(
            f'<text x="24" y="56" fill="#475569" font-size="13" font-family="Arial, sans-serif">{html_escape(subtitle)}</text>'
        )
    ...
    for index, row in enumerate(rows):
        ...
        lines.append(f'<text x="24" y="{y + 18}" fill="#0f172a" font-size="13" font-family="Arial, sans-serif">{html_escape(label)}</text>')
        lines.append(
        ...
        lines.append(
            f'<text x="{left_pad + chart_width + 12}" y="{y + 18}" fill="#0f172a" font-size="13" font-family="Arial, sans-serif">{html_escape(compact_number(value))}</text>'
        )
...
def write_plot_svgs(plots_dir: Path, analysis: dict[str, Any]) -> list[dict[str, str]]:
    ...
    plot_specs = [
        (
            ...
            render_horizontal_bar_chart_svg(
                "Ticket Status Mix",
                sorted(analysis["status_summary_rows"], key=lambda row: safe_int(row["count"]), reverse=True),
                "status",
                "count",
                subtitle="All tickets in the export.",
                bar_color="#0f766e",
            ),
            "Status mix",
        ),
        (
            ...
            render_horizontal_bar_chart_svg(
                ...
                "lab",
                "open_tickets",
                subtitle="Labs sorted by current open backlog.",
                bar_color="#2563eb",
            ),
            "Open tickets by lab",
        ),
        (
            "tickets_by_category.svg",
            render_horizontal_bar_chart_svg(
                "Tickets by Category",
                analysis["category_summary_rows"],
                "category",
                "tickets_total",
                subtitle="Total ticket volume per category.",
                bar_color="#7c3aed",
            ),
        ...
        (
            ...
            render_horizontal_bar_chart_svg(
                ...
                "priority",
                "open_count",
                subtitle="Only open tickets counted here.",
                bar_color="#dc2626",
            ),
            "Open backlog by priority",
        ),
        (
            "open_eta_buckets.svg",
            render_horizontal_bar_chart_svg(
                "Open Tickets by ETA Bucket",
                analysis["open_eta_bucket_rows"],
                "bucket",
                "count",
                subtitle="Overdue vs near-term vs long-tail work.",
                bar_color="#ea580c",
            ),
            "Open tickets by ETA bucket",
        ),
        (
            "open_age_buckets.svg",
            render_horizontal_bar_chart_svg(
                "Open Tickets by Age Bucket",
                analysis["open_age_bucket_rows"],
                "bucket",
                "count",
                subtitle="How old the currently open tickets are.",
                bar_color="#0891b2",
            ),
            "Open tickets by age bucket",
        ),
        (
            "top_commented_tickets.svg",
            render_horizontal_bar_chart_svg(
                "Top Commented Tickets",
                analysis["top_commented_rows"],
                "ticket_id",
                "comment_count",
                subtitle="Good proxy for coordination-heavy tickets.",
                bar_color="#be185d",
            ),
            "Top commented tickets",
        ),
    ...
    for filename, svg, title in plot_specs:
        (plots_dir / filename).write_text(svg, encoding="utf-8")
        manifest.append({"title": title, "file": f"plots/{filename}"})

    return manifest


def render_report(input_dir: Path, out_dir: Path, analysis: dict[str, Any], plot_manifest: list[dict[str, str]]) -> str:
    ...
    if analysis["insights"]:
        lines.extend([f"- **{item['title']}**: {item['detail']}" for item in analysis["insights"]])
    else:
        lines.append("- No standout insights detected in this export.")
    ...
    lines.extend([f"- [{item['title']}]({item['file']})" for item in plot_manifest])
...
def render_dashboard_html(input_dir: Path, out_dir: Path, analysis: dict[str, Any], plot_manifest: list[dict[str, str]]) -> str:
    summary = analysis["summary"]
    cards = [
        ("Total tickets", compact_number(summary["tickets_total"])),
        ("Open backlog", compact_number(summary["open_tickets"])),
        ...
        ("Median resolution days", compact_number(summary["approx_resolution_days_median"])),
    ]
    card_html = "".join(
        f'<div class="kpi"><div class="label">{html_escape(label)}</div><div class="value">{html_escape(value)}</div></div>'
    ...
    plot_html = "".join(
        f'<section class="plot-card"><h3>{html_escape(item["title"])}</h3><img src="{html_escape(item["file"])}" alt="{html_escape(item["title"])}" /></section>'
...
<body>
  <div class="wrap">
    <section class="hero">
      ...
      <p>Operational view built from the latest exported Firestore snapshot. It highlights backlog concentration, overdue work, aging tickets, and collaboration-heavy requests.</p>
      <div class="meta">
        <span class="pill">Source: {html_escape(str(input_dir))}</span>
        <span class="pill">Output: {html_escape(str(out_dir))}</span>
        <span class="pill">Generated: {html_escape(summary["generated_at"])}</span>
      </div>
    </section>

    <section class="kpis">{card_html}</section>

    <section class="insights">{insight_html or '<div class="insight"><h3>No standout insights</h3><p>This export looks fairly even, with no immediate hotspot above the rest.</p></div>'}</section>
    ...
    <section class="detail-grid">
      <div class="panel">
        ...
        {render_html_table(analysis["overdue_open_tickets"][:10], ["ticket_id", "lab", "priority", "overdue_days", "title"])}
      </div>
      <div class="panel">
        <h2>Comment hotspots</h2>
        {render_html_table(analysis["top_commented_rows"][:10], ["ticket_id", "lab", "status", "comment_count", "title"])}
      </div>
    </section>
  </div>
</body>
...
def main() -> int:
    ...
    (out_dir / "report.md").write_text(render_report(input_dir, out_dir, analysis, plot_manifest), encoding="utf-8")
    (out_dir / "dashboard.html").write_text(render_dashboard_html(input_dir, out_dir, analysis, plot_manifest), encoding="utf-8")
